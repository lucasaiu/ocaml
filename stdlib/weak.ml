...
external create: int -> 'a t = "caml_weak_create_r" "reentrant";;

let length x = Obj.size(Obj.repr x) - 1;;

external set : 'a t -> int -> 'a option -> unit = "caml_weak_set_r" "reentrant";;
external get: 'a t -> int -> 'a option = "caml_weak_get_r" "reentrant";;
external get_copy: 'a t -> int -> 'a option = "caml_weak_get_copy_r" "reentrant";;
external check: 'a t -> int -> bool = "caml_weak_check_r" "reentrant";;
external blit: 'a t -> int -> 'a t -> int -> int -> unit = "caml_weak_blit_r" "reentrant";;
