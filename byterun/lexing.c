...
CAMLprim value caml_lex_engine_r(CAML_R, struct lexing_table *tbl, value start_state,
  ...
  while(1) {
    ...
    if (state < 0) {
      ...
      if (lexbuf->lex_last_action == Val_int(-1)) {
        caml_failwith_r(ctx, "lexing: empty token");
      } else {
...
CAMLprim value caml_new_lex_engine_r(CAML_R, struct lexing_table *tbl, value start_state,
                                   struct lexer_buffer *lexbuf)
{
