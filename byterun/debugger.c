...
/* CAML_R: TODO. What should we do for the debugger. Multiplex the socket,
to be able to debug several threads at once, or have one socket per thread ?
Or even worse, allow only debugging of the main thread ? */
...
static void open_connection_r(CAML_R)
{
#ifdef _WIN32
...
#endif
  dbg_in = caml_open_descriptor_in_r(ctx, dbg_socket);
  dbg_out = caml_open_descriptor_out_r(ctx, dbg_socket);
  if (!caml_debugger_in_use) caml_putword_r(ctx, dbg_out, -1); /* first connection */
#ifdef _WIN32
  caml_putword_r(ctx, dbg_out, _getpid());
#else
  caml_putword_r(ctx, dbg_out, getpid());
#endif
  caml_flush_r(ctx, dbg_out);
}
...
void caml_debugger_init_r(CAML_R)
{
  ...
  caml_register_global_root_r(ctx, &marshal_flags);
  marshal_flags = caml_alloc_r(ctx, 2, Tag_cons);
  ...
  open_connection_r(ctx);
  caml_debugger_in_use = 1;
  caml_trap_barrier = caml_stack_high;
}

static value getval_r(CAML_R, struct channel *chan)
{
  value res;
  if (caml_really_getblock_r(ctx, chan, (char *) &res, sizeof(res)) == 0)
    caml_raise_end_of_file_r(ctx); /* Bad, but consistent with caml_getword */
  return res;
}

static void putval_r(CAML_R, struct channel *chan, value val)
{
  caml_really_putblock_r(ctx, chan, (char *) &val, sizeof(val));
}

static void safe_output_value_r(CAML_R, struct channel *chan, value val)
{
  ...
  if (sigsetjmp(raise_buf.buf, 0) == 0) {
    ...
    caml_output_val_r(ctx, chan, val, marshal_flags);
  } else {
    /* Send wrong magic number, will cause [caml_input_value] to fail */
    caml_really_putblock_r(ctx,chan, "\000\000\000\000", 4);
  }
...
void caml_debugger_r(CAML_R, enum event_kind event)
{
  ...
  caml_putword_r(ctx, dbg_out, caml_event_count);
  if (event == EVENT_COUNT || event == BREAKPOINT) {
    caml_putword_r(ctx,dbg_out, caml_stack_high - frame);
    caml_putword_r(ctx,dbg_out, (Pc(frame) - caml_start_code) * sizeof(opcode_t));
  } else {
    /* No PC and no stack frame associated with other events */
    caml_putword_r(ctx,dbg_out, 0);
    caml_putword_r(ctx,dbg_out, 0);
  }
  caml_flush_r(ctx, dbg_out);
 ...
 command_loop:
  ...
  while(1) {
    switch(getch(dbg_in)) {
    case REQ_SET_EVENT:
      pos = caml_getword_r(ctx, dbg_in);
      Assert (pos >= 0);
      Assert (pos < caml_code_size);
      caml_set_instruction_r(ctx, caml_start_code + pos / sizeof(opcode_t), EVENT);
      break;
    case REQ_SET_BREAKPOINT:
      pos = caml_getword_r(ctx, dbg_in);
      Assert (pos >= 0);
      Assert (pos < caml_code_size);
      caml_set_instruction_r(ctx, caml_start_code + pos / sizeof(opcode_t), BREAK);
      break;
    case REQ_RESET_INSTR:
      ...
      caml_set_instruction_r(ctx, caml_start_code + pos, caml_saved_code[pos]);
      break;
    case REQ_CHECKPOINT:
#ifndef _WIN32
      i = fork();
      if (i == 0) {
        ...
        open_connection_r(ctx);      /* Open new connection with debugger */
      } else {
        caml_putword_r(ctx,dbg_out, i);
        caml_flush_r(ctx, dbg_out);
      }
#else
      ...
      break;
    case REQ_GO:
      caml_event_count = caml_getword_r(ctx,dbg_in);
      return;
    case REQ_STOP:
      exit(0);
      break;
    ...
    case REQ_GET_LOCAL:
      i = caml_getword_r(ctx,dbg_in);
      putval_r(ctx, dbg_out, Locals(frame)[i]);
      caml_flush_r(ctx,dbg_out);
      break;
    case REQ_GET_ENVIRONMENT:
      i = caml_getword_r(ctx,dbg_in);
      putval_r(ctx,dbg_out, Field(Env(frame), i));
      caml_flush_r(ctx,dbg_out);
      break;
    case REQ_GET_GLOBAL:
      i = caml_getword_r(ctx,dbg_in);
      putval_r(ctx,dbg_out, Field(caml_global_data, i));
      caml_flush_r(ctx,dbg_out);
      break;
    case REQ_GET_ACCU:
      putval_r(ctx,dbg_out, *caml_extern_sp);
      caml_flush_r(ctx,dbg_out);
      break;
    case REQ_GET_HEADER:
      val = getval_r(ctx, dbg_in);
      caml_putword_r(ctx,dbg_out, Hd_val(val));
      caml_flush_r(ctx,dbg_out);
      break;
    case REQ_GET_FIELD:
      val = getval_r(ctx, dbg_in);
      ...
      if (Tag_val(val) != Double_array_tag) {
        putch(dbg_out, 0);
        putval_r(ctx,dbg_out, Field(val, i));
      } else {
        double d = Double_field(val, i);
        putch(dbg_out, 1);
        caml_really_putblock_r(ctx, dbg_out, (char *) &d, 8);
      }
      caml_flush_r(ctx,dbg_out);
      break;
    case REQ_MARSHAL_OBJ:
      val = getval_r(ctx, dbg_in);
      safe_output_value_r(ctx, dbg_out, val);
      caml_flush_r(ctx,dbg_out);
      break;
    case REQ_GET_CLOSURE_CODE:
      val = getval_r(ctx,dbg_in);
      caml_putword_r(ctx,dbg_out, (Code_val(val)-caml_start_code) * sizeof(opcode_t));
      caml_flush_r(ctx,dbg_out);
      break;
    case REQ_SET_FORK_MODE:
      caml_debugger_fork_mode = caml_getword_r(ctx,dbg_in);
      break;
    }
  }
}
