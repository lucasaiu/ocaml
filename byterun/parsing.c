...
CAMLprim value caml_parse_engine_r(CAML_R, struct parser_tables *tables,
                                 struct parser_env *env, value cmd, value arg)
{
  ...
  case TOKEN_READ:
    ...
    if (Is_block(arg)) {
      ...
      caml_modify_r(ctx,&env->lval, Field(arg, 0));
    } else {
      env->curr_char = Field(tables->transl_const, Int_val(arg));
      caml_modify_r(ctx, &env->lval, Val_long(0));
    }
  ...
  push:
    ...
    caml_modify_r(ctx, &Field(env->v_stack, sp), env->lval);
  ...
  case SEMANTIC_ACTION_COMPUTED:
    RESTORE;
    Field(env->s_stack, sp) = Val_int(state);
    caml_modify_r(ctx, &Field(env->v_stack, sp), arg);
    asp = Int_val(env->asp);
...
CAMLprim value caml_set_parser_trace_r(CAML_R, value flag)
{
