...
static int random_level_r(CAML_R)
{
  uint32 r;
  int level = 0;
...
static void caml_insert_global_root_r(CAML_R, struct global_root_list * rootlist,
                                    value * r)
{
...
static void caml_delete_global_root_r(CAML_R, struct global_root_list * rootlist,
                                    value * r)
{
  struct global_root * update[NUM_LEVELS];
  struct global_root * e, * f;
...
static void caml_iterate_global_roots_r(CAML_R, scanning_action f,
...
static void caml_empty_global_roots_r(CAML_R, struct global_root_list * rootlist)
{
...
CAMLexport void caml_register_global_root_r(CAML_R, value *r)
{
...
CAMLexport void caml_remove_global_root_r(CAML_R, value *r)
{
  caml_delete_global_root_r(ctx, &caml_global_roots, r);
}
...
{
  /* Trace OCaml context-local globals: */
  caml_scan_caml_globals_r(ctx, f);

  caml_iterate_global_roots_r(ctx, f, &caml_global_roots);
  caml_iterate_global_roots_r(ctx, f, &caml_global_roots_young);
  caml_iterate_global_roots_r(ctx, f, &caml_global_roots_old);
}
...
void caml_scan_global_young_roots_r(CAML_R, scanning_action f)
{
  ...
  for (gr = caml_global_roots_young.forward[0];
    ...
    caml_insert_global_root_r(ctx, &caml_global_roots_old, gr->root);
  }
  caml_empty_global_roots_r(ctx, &caml_global_roots_young);
}
