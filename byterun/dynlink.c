...
static c_primitive lookup_primitive_r(CAML_R, char * name)
{
  //printf("!!!!Looking up the primitive >%s<\n", name);
  int i;
  void * res;

  for (i = 0; caml_names_of_builtin_cprim[i] != NULL; i++) {
    //printf("!!!!Looking up the primitive >%s<: is it >%s<?\n", name, caml_names_of_builtin_cprim[i]);
    if (strcmp(name, caml_names_of_builtin_cprim[i]) == 0)
      return caml_builtin_cprim[i];
  }
  for (i = 0; i < caml_shared_libs.size; i++) {
    res = caml_dlsym(caml_shared_libs.contents[i], name);
...
static char * parse_ld_conf_r(CAML_R)
{
...
/* Open the given shared library and add it to shared_libs.
   Abort on error. */
static void open_shared_lib_r(CAML_R, char * name)
{
  char * realname;
  void * handle;
  ...
  if (handle == NULL)
    caml_fatal_error_arg2("Fatal error: cannot load shared library %s\n", name,
                          "Reason: %s\n", caml_dlerror());
  caml_ext_table_add(&caml_shared_libs, handle);
...
void caml_build_primitive_table_r(CAML_R, char * lib_path,
                                char * libs,
                                char * req_prims)
{
  ...
  tofree2 = parse_ld_conf_r(ctx);
  /* Open the shared libraries */
  caml_ext_table_init(&caml_shared_libs, 8);
  if (libs != NULL)
    for (p = libs; *p != 0; p += strlen(p) + 1)
      open_shared_lib_r(ctx, p);
  ...
  for (p = req_prims; *p != 0; p += strlen(p) + 1) {
    c_primitive prim = lookup_primitive_r(ctx, p);
    if (prim == NULL)
...
void caml_build_primitive_table_builtin_r(CAML_R)
{
  int i;
  caml_ext_table_init(&caml_prim_table, 0x180);
#ifdef DEBUG
...
CAMLprim value caml_dynlink_open_lib_r(CAML_R, value mode, value filename)
{
  void * handle;
  value result;
  ...
  if (handle == NULL) caml_failwith_r(ctx, caml_dlerror());
  result = caml_alloc_small_r(ctx, 1, Abstract_tag);
...
/*#include <stdio.h>*/
CAMLprim value caml_dynlink_lookup_symbol_r(CAML_R, value handle, value symbolname)
{
  void * symb;
  value result;
...
CAMLprim value caml_dynlink_add_primitive_r(CAML_R, value handle)
{
  return Val_int(caml_ext_table_add(&caml_prim_table, Handle_val(handle)));
}

CAMLprim value caml_dynlink_get_current_libs_r(CAML_R, value unit)
{
  CAMLparam0();
  CAMLlocal1(res);
  int i;

  res = caml_alloc_tuple_r(ctx, caml_shared_libs.size);
  for (i = 0; i < caml_shared_libs.size; i++) {
    value v = caml_alloc_small_r(ctx, 1, Abstract_tag);
    Handle_val(v) = caml_shared_libs.contents[i];
    Store_field(res, i, v);
  }
  CAMLreturn(res);
}
...
value caml_dynlink_add_primitive_r(CAML_R, value handle)
{
  caml_invalid_argument_r(ctx, "dynlink_add_primitive");
  return Val_unit; /* not reached */
}

value caml_dynlink_get_current_libs_r(CAML_R, value unit)
{
  caml_invalid_argument_r(ctx, "dynlink_get_current_libs");
  return Val_unit; /* not reached */
}
